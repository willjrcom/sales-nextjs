...
export default function EmployeeAdminPage() {
  ...
  const empReports = reportConfigs.filter((c) => reportIds.includes(c.id));
  const today = new Date().toISOString().slice(0, 10);
  const initialDates = empReports.reduce((acc, cur) => {
  ...
  return (
    <div className="p-4 ml-52">
      <PageTitle title="Funcionários" tooltip="Relatórios de Funcionários e Entregadores" />
      {empReports.map((config) => {
