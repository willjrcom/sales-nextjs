...
export default function ProductAdminPage() {
  ...
  const prodReports = reportConfigs.filter((c) => reportIds.includes(c.id));
  const today = new Date().toISOString().slice(0, 10);
  const initialDates = prodReports.reduce((acc, cur) => {
  ...
  return (
    <div className="p-4 ml-52">
      <PageTitle title="Produtos" tooltip="Relatórios de Produtos" />
      {prodReports.map((config) => {
