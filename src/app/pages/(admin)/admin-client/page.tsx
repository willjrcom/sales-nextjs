...
export default function ClientAdminPage() {
  ...
  const clientReports = reportConfigs.filter((c) => reportIds.includes(c.id));
  const today = new Date().toISOString().slice(0, 10);
  const initialDates = clientReports.reduce((acc, cur) => {
  ...
  return (
    <div className="p-4 ml-52">
      <PageTitle title="Clientes" tooltip="Relatórios de Clientes" />
      {clientReports.map((config) => {
