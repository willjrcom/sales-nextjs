...
export default function ShiftAdminPage() {
  ...
  const shiftReports = reportConfigs.filter((c) => reportIds.includes(c.id));
  const today = new Date().toISOString().slice(0, 10);
  const initialDates = shiftReports.reduce((acc, cur) => {
  ...
  return (
    <div className="p-4 ml-52">
      <PageTitle title="Turnos" tooltip="Relatórios de Turnos" />
      {shiftReports.map((config) => {
