...
export default function OrderAdminPage() {
  ...
  const orderReports = reportConfigs.filter((c) => reportIds.includes(c.id));
  const today = new Date().toISOString().slice(0, 10);
  const initialDates = orderReports.reduce((acc, cur) => {
  ...
  return (
    <div className="p-4 ml-52">
      <PageTitle title="Pedidos" tooltip="Relatórios de Pedidos" />
      {orderReports.map((config) => {
