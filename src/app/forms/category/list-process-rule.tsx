...
const SortableItem = ({ id, rule, category }: SortableItemProps) => {
    ...
    return (
        <div
            ...
            style={style}
            className={cn(
                "group relative bg-[#1c2431] border border-white/5 rounded-xl p-4 mb-3 transition-all duration-200",
                "hover:border-blue-500/30 hover:bg-[#232d3d] shadow-sm",
                isDragging && "z-50 opacity-50 scale-[1.02] border-blue-500 bg-[#232d3d] shadow-2xl"
            )}
        >
            <div className="flex items-center gap-4">
                ...
                <div
                    {...attributes}
                    {...listeners}
                    className="cursor-grab active:cursor-grabbing p-1 text-white/20 group-hover:text-white/40 transition-colors"
                >
                    <GripVertical size={20} />
                </div>

                {/* Icon/Avatar */}
                <div className="h-12 w-12 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 border border-blue-500/20">
                    <ChefHat size={24} />
                </div>
                    ...
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button
                                variant="ghost"
                                size="icon"
                                className="text-white/30 hover:text-white hover:bg-white/5 rounded-full"
                            >
                                <Settings2 size={18} />
                            </Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                            ...
                            <ProcessRuleForm item={rule} isUpdate category={category} />
                        </DialogContent>
                    </Dialog>
                </div>
            </div>
    ...
    return (
        <div className="relative min-h-[400px]">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-white mb-1">Fluxo de Preparo</h2>
                    <p className="text-sm text-white/40">Arraste para redefinir a sequência dos processos.</p>
                </div>
            ...
            {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-white/5 rounded-2xl bg-white/[0.02]">
                    <div className="h-16 w-16 rounded-full bg-white/5 flex items-center justify-center text-white/20 mb-4">
                        <AlertCircle size={32} />
                    </div>
                    <p className="text-white/60 font-medium">Nenhuma etapa definida</p>
                    <p className="text-white/30 text-sm">Comece adicionando o primeiro processo do fluxo.</p>
                </div>
            ) : (
                <DndContext
                    sensors={sensors}
