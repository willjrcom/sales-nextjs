...
function getStatusBadge(order: Order): { label: string; variant: BadgeVariant } {
    switch (order.status) {
        case "Staging":
            return { label: "Em separação", variant: "gray" };
        case "Pending":
            return { label: "Pendente", variant: "yellow" };
        case "Ready":
            return { label: "Pronto", variant: "green" };
        case "Archived":
            return { label: "Arquivado", variant: "red" };
        case "Canceled":
            return { label: "Cancelado", variant: "red" };
        case "Finished":
            return { label: "Finalizado", variant: "green" };
        default:
            return { label: String(order.status), variant: "gray" };
    }
}
